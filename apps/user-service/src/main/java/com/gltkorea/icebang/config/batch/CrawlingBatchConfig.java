...
/**
 * 외부 크롤링 API를 호출하여 상품 정보를 업데이트하는 배치(Batch) 작업을 설정
 * Job은 병렬 처리와 내결함성(Fault Tolerance) 정책을 적용해 안정적인 운영 환경을 고려
 */
@Slf4j
@Configuration
...
public class CrawlingBatchConfig {
    ...
    /**
     * 'crawlingJob'이라는 이름의 Batch Job을 정의
     * Job은 배치 작업의 가장 큰 단위이며, 하나 이상의 Step으로 구성
     */
    @Bean
    ...
    /**
     * 'crawlingStep'이라는 이름의 Batch Step을 정의
     * Step은 Reader, Processor, Writer의 흐름으로 구성되며,
     * 이 Step은 병렬 처리와 실패 처리 정책이 적용
     */
    @Bean
    ...
    /**
     * ItemReader를 스레드로부터 안전하게 만들기 위해 SynchronizedItemStreamReader로 감쌈
     * 병렬 처리 시 여러 스레드가 Reader에 동시에 접근하는 것을 방지
     */
    @Bean
    public SynchronizedItemStreamReader<ProductCrawlingData> synchronizedProductReader() {
        ...
        return new SynchronizedItemStreamReaderBuilder<ProductCrawlingData>()
                .delegate(reader)
                .build();
    }

    /**
     * 실제 DB에서 크롤링할 상품 목록을 읽어오는 ItemReader
     * MyBatisPagingItemReader는 MyBatis를 사용하여 페이징 기반으로 대용량 데이터를 안전하게 읽어옴
     */
    ...
    /**
     * ItemProcessor: Reader가 읽어온 각 아이템을 가공
     * 여기서는 FastAPI 크롤링 서버를 호출하여 추가 정보를 얻어옴
     * 예외를 직접 처리(try-catch)하지 않고 Step으로 던져서, Step의 재시도/건너뛰기 정책이 동작하도록 함
     */
    @Bean
    public ItemProcessor<ProductCrawlingData, ProductCrawlingData> crawlingProcessor() {
        return product -> {
            ...
            ResponseEntity<FastApiCrawlingResponse> response = restTemplate.getForEntity(fastApiUrl, FastApiCrawlingResponse.class);
            ...
            log.warn("Crawling API call returned non-successful status for product {}: {}", product.getName(), response.getStatusCode());
            return null; // 이 아이템만 건너뜀 (Writer로 전달되지 않음)
        };
    }

    /**
     * ItemWriter: 가공된 데이터 묶음(Chunk)을 DB에 일괄 저장
     * MyBatisBatchItemWriter는 내부적으로 JDBC Batch Update를 사용하여 성능이 좋음
     */
    @Bean
