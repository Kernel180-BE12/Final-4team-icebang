 ...
 * - 실제 SQL은 UserMapper.xml에 작성
 * - Spring이 자동으로 구현체를 생성해줌
 * - 각 메서드는 XML의 동일한 id와 매핑됨
 */
...
public interface UserMapper {
  ...
  /**
   * 이메일로 사용자 조회 (기본 정보만)
   * - 로그인 시 이메일을 ID로 사용하므로 중요
   * - userEmail 필드로 검색
   * - 이메일은 유니크하므로 결과는 0개 또는 1개
   *
  ...
  /**
   * 사용자와 조직 정보를 함께 조회 (JOIN 사용)
   * - USERS + DEPARTMENT + POSITION + ORGANIZATION 테이블 조인
   * - 상세 정보가 필요한 경우 사용 (마이페이지, 사용자 목록 등)
   * - 조회 성능은 떨어지지만 한 번에 모든 정보 획득
   *
   * @param username 검색할 사용자명
  ...
  /**
   * 이메일로 상세 정보 조회 (JOIN 사용)
   * - 이메일 기반 로그인 후 상세 정보가 필요할 때 사용
   *
   * @param email 검색할 이메일 주소
   * @return 사용자 정보 + 부서명, 직급명, 조직명 포함
   */
  ...
  /**
   * 사용자의 역할 목록 조회
   * - Spring Security에서 권한 검사 시 사용
   * - USERS_ROLE + ROLE 테이블 조인
   * - 결과: ["ADMIN", "DATA_ENGINEER"] 형태
   *
  ...
  /**
   * 사용자의 권한 목록 조회
   * - 역할을 통해 간접적으로 가진 권한들을 조회
   * - USERS_ROLE + ROLE_PERMISSION + PERMISSION 테이블 조인
   ...
   * 새 사용자 계정 생성
   * - 관리자가 신규 계정 생성 시 사용
   * - userId는 AUTO_INCREMENT로 자동 생성됨
   * - useGeneratedKeys=true로 생성된 ID를 다시 받아올 수 있음
   *
   ...
   * 사용자 계정 상태만 변경
   * - 계정 활성화/정지 등에 사용
   * - ACTIVE, SUSPENDED, INACTIVE 등으로 변경
   *
   * @param userId 수정할 사용자 ID
  ...
  /**
   * 사용자 계정 삭제
   * - 실제 운영에서는 soft delete(상태 변경) 권장
   * - 외래키 제약 조건 때문에 USERS_ROLE 먼저 삭제 필요
   *
