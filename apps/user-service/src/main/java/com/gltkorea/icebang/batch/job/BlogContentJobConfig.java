...
public class BlogContentJobConfig {
    ...
    @Bean
    public Job blogContentJob(JobRepository jobRepository, Step keywordExtractionStep, Step contentGenerationStep) {
    ...
    @Bean
    public Step keywordExtractionStep(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
        return new StepBuilder("keywordExtractionStep", jobRepository) // 변경점 3: StepBuilder를 직접 생성합니다.
                .tasklet(keywordExtractionTasklet, transactionManager) // 변경점 4: tasklet에 transactionManager를 함께 전달합니다.
                .build();
    }

    @Bean
    public Step contentGenerationStep(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
