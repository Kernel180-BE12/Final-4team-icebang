 ...
 * - MyBatis에서 조회/수정 시 사용하는 Java 객체
 */
...
public class Users {
   ...
   * - DB에서 AUTO_INCREMENT로 자동 생성
   * - 시스템 내에서 사용자를 구분하는 유일한 값
   */
  private Long userId;
   ...
   * 사용자 이름
   * - 화면에 표시되는 이름 (예: "홍길동")
   * - 관리자가 계정 생성 시 설정하거나, 사용자가 마이페이지에서 수정
   */
  ...
  /**
   * 사용자 비밀번호 (암호화된 상태로 저장)
   * - BCrypt로 암호화되어 저장됨
   * - 관리자가 계정 생성 시 임시 비밀번호 설정
   * - 사용자가 마이페이지에서 변경 가능
   */
   ...
   * 사용자 계정 상태
   * 가능한 값:
   * - "ACTIVE": 정상 활성 상태 (로그인 가능)
   * - "SUSPENDED": 계정 정지 (로그인 불가)
   * - "INACTIVE": 비활성 상태 (휴면 계정)
   * - "PENDING": 대기 상태 (아직 활성화 안됨)
   */
  private String userStatus;

  /**
   * 소속 부서 ID
   * - DEPARTMENT 테이블의 dept_id와 연결
   * - 관리자가 계정 생성 시 설정
   */
  private Long deptId;
  ...
  /**
   * 부서명 (조인 데이터)
   * - DEPARTMENT 테이블에서 가져온 dept_name
   * - 사용자 정보 조회 시 함께 표시하기 위해 사용
   */
  private String deptName;

  /**
   * 직급명 (조인 데이터)
   * - POSITION 테이블에서 가져온 position_title
   * - 사용자 정보 조회 시 함께 표시하기 위해 사용
   */
  ...
  /**
   * 조직명 (조인 데이터)
   * - ORGANIZATION 테이블에서 가져온 org_name
   * - 사용자 정보 조회 시 함께 표시하기 위해 사용
   */
