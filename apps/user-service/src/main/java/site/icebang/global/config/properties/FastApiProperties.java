 ...
 * {@code @Validated}와 {@code @NotBlank}를 사용하여, 필수 설정값(url)이 누락될 경우 애플리케이션 시작 시점에
 * 즉시 에러를 발생시켜 설정 오류를 방지합니다.
 *
 * <h2>사용 예제:</h2>
 * <pre>{@code
 * @Component
...
public class FastApiProperties {
   ...
   * <p>
   * {@code @NotBlank} 어노테이션이 적용되어 있어, application.yml 파일에 반드시 값이 존재해야 합니다.
   * (예: "http://host.docker.internal:8000")
   */
