 ...
 *
 * <p>이 클래스는 내부 시스템의 다른 부분들이 외부 시스템의 상세한 통신 방법을 알 필요가 없도록
 * HTTP 요청/응답 로직을 캡슐화합니다. {@code RestTemplate}을 사용하여 실제 통신을 수행하며,
 * 모든 FastAPI 요청은 이 클래스의 {@code call} 메소드를 통해 이루어져야 합니다.
 *
 * <h2>사용 예제:</h2>
 * <pre>{@code
 * @Autowired
...
public class FastApiAdapter {
   ...
   *
   * <p>지정된 엔드포인트, HTTP 메소드, 요청 Body를 사용하여 외부 API를 호출합니다.
   * 통신 성공 시 응답 Body를 문자열로 반환하고, 실패 시 에러 로그를 남기고 null을 반환합니다.
   *
