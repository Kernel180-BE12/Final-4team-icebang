 ...
 *
 * <p>이 클래스는 {@code TaskRunner} 인터페이스를 구현하며, Task의 타입이 'FastAPI'일 때 선택됩니다.
 * 실제 HTTP 통신은 {@code FastApiAdapter}에 위임하고, 이 클래스는 워크플로우의 {@code Task} 객체를
 * {@code FastApiAdapter}가 이해할 수 있는 호출 형식으로 변환하는 **어댑터(Adapter)** 역할을 수행합니다.
 *
 * <h2>주요 기능:</h2>
 * <ul>
 * <li>Task 파라미터에서 endpoint와 method 정보 파싱</li>
 * <li>사전에 생성된 Request Body를 {@code FastApiAdapter}에 전달하여 실행 위임</li>
 * <li>어댑터의 실행 결과를 {@code TaskExecutionResult} 형식으로 변환하여 반환</li>
 * </ul>
 *
...
public class FastApiTaskRunner implements TaskRunner {
   ...
   *
   * <p>Task의 파라미터에서 엔드포인트와 HTTP 메소드를 추출하고,
   * {@code WorkflowExecutionService}로부터 전달받은 동적 Request Body를 사용하여
   * {@code FastApiAdapter}를 호출합니다.
   *
