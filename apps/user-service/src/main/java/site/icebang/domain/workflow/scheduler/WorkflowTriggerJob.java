 ...
 *
 * <p>이 클래스는 Quartz의 스케줄링 세계와 애플리케이션의 비즈니스 로직을 연결하는
 * **브릿지(Bridge)** 역할을 수행합니다. Quartz의 Trigger가 정해진 시간에 발동하면,
 * Quartz 엔진은 이 Job을 인스턴스화하고 {@code executeInternal} 메소드를 호출합니다.
 *
 * <h2>주요 기능:</h2>
 * <ul>
 * <li>스케줄 실행 시점에 {@code JobDataMap}에서 실행할 워크플로우 ID를 추출</li>
 * <li>추출된 ID를 사용하여 {@code WorkflowExecutionService}를 호출하여 실제 워크플로우 실행을 위임</li>
 * </ul>
 *
...
public class WorkflowTriggerJob extends QuartzJobBean {
   ...
   *
   * <p>이 메소드는 실행 컨텍스트({@code JobExecutionContext})에서 {@code JobDataMap}을 통해
   * 스케줄 등록 시점에 저장된 'workflowId'를 추출합니다. 그 후, 해당 ID를 파라미터로 하여
   * {@code WorkflowExecutionService}의 {@code executeWorkflow} 메소드를 호출하여
   * 실제 비즈니스 로직의 실행을 시작합니다.
   *
