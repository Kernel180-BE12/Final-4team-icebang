 ...
 *
 * <p>이 서비스는 워크플로우 실행 목록의 페이징 처리, 특정 실행 건의 상세 정보 조회 등
 * 읽기 전용(Read-Only) 기능에 집중합니다.
 *
 * <h2>주요 기능:</h2>
 * <ul>
 * <li>워크플로우 실행 이력 목록 페이징 조회</li>
 * <li>워크플로우 실행 상세 정보 조회 (Job 및 Task 실행 이력 포함)</li>
 * </ul>
 *
...
public class WorkflowHistoryService implements PageableService<WorkflowHistoryDTO> {
   ...
   *
   * <p>지정된 실행 ID(`runId`)에 해당하는 워크플로우 실행 정보와, 그에 속한 모든 Job 실행 정보,
   * 그리고 각 Job에 속한 모든 Task 실행 정보를 계층적으로 조회하여 반환합니다.
   *
