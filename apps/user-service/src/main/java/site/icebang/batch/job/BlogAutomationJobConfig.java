...
import site.icebang.batch.tasklet.*; // import 경로 수정
...
/**
 * [배치 시스템 구현]
 * 트렌드 기반 블로그 자동화 워크플로우를 구성하는 Job들을 정의합니다.
 */
@Configuration
...
public class BlogAutomationJobConfig {
  ...
  /**
   * Job 1: 상품 선정 및 정보 수집
   * 키워드 추출부터 최종 상품 정보 크롤링까지의 과정을 책임집니다.
   */
  @Bean
  public Job productSelectionJob(JobRepository jobRepository,
  ...
  /**
   * Job 2: 콘텐츠 생성 및 발행
   * 수집된 상품 정보로 블로그 콘텐츠를 생성하고 발행합니다.
   */
  @Bean
  public Job contentPublishingJob(JobRepository jobRepository,
                                  Step generateBlogContentStep,
                                  Step publishBlogPostStep) {
  ...
  @Bean
  public Step extractTrendKeywordStep(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
  ...
  @Bean
  public Step searchProductsFromMallStep(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
  ...
  @Bean
  public Step matchProductWithKeywordStep(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
  ...
  @Bean
  public Step findSimilarProductsStep(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
  ...
  @Bean
  public Step crawlSelectedProductStep(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
  ...
  @Bean
  public Step generateBlogContentStep(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
  ...
  @Bean
  public Step publishBlogPostStep(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
