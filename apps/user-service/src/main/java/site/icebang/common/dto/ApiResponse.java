 ...
 * <p>
 *    REST API의 응답 형식을 표준화하기 위해 사용됩니다.
 *    모든 응답은 성공 여부({@link #success}), 응답 데이터({@link #data}),
 *    응답 메시지({@link #message}), 그리고 HTTP 상태 코드({@link #status})를 포함합니다.
 * </p>
 *
 * <p><b>사용 예시:</b></p>
 * <pre>{@code
...
public class ApiResponse<T> {
   ...
   * 실제 응답 데이터(payload).
   * <p>
   * 요청이 성공적으로 처리되었을 경우 반환되는 데이터이며,
   * 실패 시에는 {@code null}일 수 있습니다.
   * </p>
   */
  private T data;
   ...
   * 응답 메세지.
   * <p>
   *     성공 또는 오류 상황을 설명하는 메시지를 담습니다.
   *     클라이언트에서 사용자에게 직접 표시할 수도 있습니다.
   * </p>
   */
