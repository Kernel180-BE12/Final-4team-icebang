...
class CrawlService:
    ...
    async def crawl_product_detail(self, request: RequestSadaguCrawl) -> dict:
        ...
        try:
            logger.info(f"상품 상세 크롤링 서비스 시작: job_id={request.job_id}, schedule_id={request.schedule_id}, product_url={request.product_url}")
            ...
            logger.success(f"크롤링 완료: title='{product_title}', price={product_detail.get('price', 0)}, options_count={len(product_detail.get('options', []))}")
            ...
            logger.info(f"상품 상세 크롤링 서비스 완료: job_id={request.job_id}, status=success")
            return response_data

        except Exception as e:
            logger.error(f"크롤링 서비스 오류: job_id={request.job_id}, product_url={request.product_url}, error='{e}'")
