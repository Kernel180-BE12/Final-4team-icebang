...
class SimilarityService:
    ...
    def select_product_by_similarity(self, request: RequestSadaguSimilarity) -> dict:
        ...
        logger.info(f"유사도 분석 서비스 시작: job_id={request.job_id}, keyword='{keyword}', matched_count={len(candidates) if candidates else 0}, fallback_count={len(fallback_products)}")

        # 매칭된 상품이 없으면 전체 검색 결과로 폴백
        if not candidates:
            if not fallback_products:
                logger.warning(f"매칭된 상품과 검색 결과가 모두 없음: keyword='{keyword}'")
                return {
        ...
        try:
            ...
            logger.info(f"키워드 '{keyword}'와 {len(candidates)}개 상품의 유사도 분석 시작... (모드: {analysis_mode})")
            ...
            if len(candidates) == 1:
                ...
                if analysis_mode == "fallback_similarity_only":
                    ...
                    if similarity < similarity_threshold:
                        logger.warning(f"단일 상품 유사도 미달: similarity={similarity:.4f} < threshold={similarity_threshold}")
                        return {
                            "job_id": request.job_id,
                            "schedule_id": request.schedule_id,
                            "schedule_his_id": request.schedule_his_id,
                ...
                logger.success(f"단일 상품 선택 완료: title='{selected_product['title'][:30]}', similarity={similarity:.4f}")
            ...
            for i, result in enumerate(similarity_results[:5]):  # 상위 5개만 로그
                logger.info(f"  {i+1}위: {result['title'][:40]} | 유사도: {result['similarity']:.4f}")
            ...
            if analysis_mode == "fallback_similarity_only" and best_result['similarity'] < similarity_threshold:
                logger.warning(f"최고 유사도 미달: similarity={best_result['similarity']:.4f} < threshold={similarity_threshold}")
                return {
                    "job_id": request.job_id,
                    "schedule_id": request.schedule_id,
                    "schedule_his_id": request.schedule_his_id,
            ...
            if analysis_mode == "matched_products" and 'match_info' in selected_product:
                ...
                logger.info(f"종합 점수 계산: match_score={match_score:.4f}, similarity_score={similarity_score:.4f}, final_score={final_score:.4f}")
            else:
                reason = f"유사도({best_result['similarity']:.4f}) 기준 선택 ({analysis_mode})"

            logger.success(f"상품 선택 완료: title='{selected_product['title'][:30]}', {reason}")
            ...
            logger.error(f"유사도 분석 서비스 오류: job_id={request.job_id}, keyword='{keyword}', error='{e}'")
