...
class MatchService:
    ...
    def match_products(self, request: RequestSadaguMatch) -> dict:
        ...
        logger.info(f"키워드 매칭 서비스 시작: job_id={request.job_id}, schedule_id={request.schedule_id}, keyword='{keyword}', products_count={len(products) if products else 0}")
        ...
        try:
            ...
            for i, product in enumerate(products):
                ...
                if match_result['is_match']:
                    ...
                    logger.info(f"상품 {i + 1} 매칭 성공: title='{title[:30]}', type={match_result['match_type']}, score={match_result['score']:.3f}")
            ...
            if matched_products:
                ...
                logger.info(f"최고 매칭 상품: title='{best_match['title'][:30]}', score={best_match['match_info']['match_score']:.3f}")
            ...
            logger.error(f"매칭 서비스 오류: job_id={request.job_id}, keyword='{keyword}', error='{e}'")
