...
class SimilarityAnalyzer:
    ...
    def calculate_similarity(self, text1: str, text2: str) -> float:
        ...
        try:
            ...
            logger.error(f"유사도 계산 오류: text1='{text1[:30]}', text2='{text2[:30]}', error='{e}'")
            raise
    ...
    def analyze_similarity_batch(self, keyword: str, product_titles: list[str]) -> list[dict]:
        ...
        try:
            ...
            for i, title in enumerate(product_titles):
                try:
                    logger.debug(f"유사도 계산 중 ({i + 1}/{len(product_titles)}): title='{title[:30]}'")
                    ...
                    logger.debug(f"유사도 계산 완료 ({i + 1}/{len(product_titles)}): similarity={similarity:.4f}")
                except Exception as e:
