...
class S3ImageInfo(BaseModel):
    ...
    file_size: int = Field(
        ..., title="파일 크기", description="업로드된 파일 크기 (bytes)"
    )
...
class ProductS3UploadResult(BaseModel):
    ...
    upload_folder: Optional[str] = Field(
        None, title="업로드 폴더", description="S3 내 상품별 폴더 경로"
    )
    folder_s3_url: Optional[str] = Field(
        None, title="폴더 S3 URL", description="S3 폴더 접근 URL"
    )
    ...
    reason: Optional[str] = Field(
        None, title="건너뜀 사유", description="업로드를 건너뛴 이유"
    )
    error: Optional[str] = Field(
        None, title="오류 메시지", description="업로드 중 발생한 오류"
    )
...
class S3UploadSummary(BaseModel):
    ...
    processed_products: int = Field(
        ..., title="처리된 상품 수", description="실제 처리된 상품 수"
    )
    skipped_products: int = Field(
        ..., title="건너뛴 상품 수", description="크롤링 실패로 건너뛴 상품 수"
    )
    ...
    success_rate: str = Field(
        ..., title="성공률", description="이미지 업로드 성공률 (성공/전체)"
    )
...
class S3UploadData(BaseModel):
    ...
    base_folder: str = Field(
        ..., title="기본 폴더", description="S3 업로드에 사용된 기본 폴더"
    )
