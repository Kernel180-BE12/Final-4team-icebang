...
# Jenkins Freestyle Project Script for Python Deployment (FastAPI)
# This script replaces the GitHub Actions workflow: .github/workflows/deploy-fastapi.yml

echo "=============================="
echo "FastAPI Deployment to AWS EC2"
echo "=============================="
...
# FASTAPI_SERVER_HOST - Target server host
# SERVER_SSH_KEY - SSH private key for server access
# GITHUB_TOKEN - GitHub token for Docker registry
# GITHUB_USERNAME - GitHub username
# DISCORD_WEBHOOK_URL - Discord webhook for notifications

# Check if this is a successful CI build for a pre-processing tag
if [[ ! "${GIT_BRANCH}" =~ ^origin/tags/pre-processing-v.* ]]; then
    echo "Not a pre-processing tag build, skipping deployment"
    exit 0
fi
...
echo "Deploying to FastAPI server: ${FASTAPI_SERVER_HOST}"
...
# Copy docker compose files to FastAPI EC2
echo "Copying docker compose files to server..."
scp -i ~/.ssh/deploy_key -o StrictHostKeyChecking=no \
    docker/production-fastapi/docker-compose.yml \
    ubuntu@${FASTAPI_SERVER_HOST}:~/app/docker/production-fastapi/

# Copy .env.prod file to FastAPI EC2
...
scp -i ~/.ssh/deploy_key -o StrictHostKeyChecking=no \
    docker/production-fastapi/promtail-config.yml \
    ubuntu@${FASTAPI_SERVER_HOST}:~/app/docker/production-fastapi/

scp -i ~/.ssh/deploy_key -o StrictHostKeyChecking=no \
    docker/production-fastapi/agent-config.yml \
    ubuntu@${FASTAPI_SERVER_HOST}:~/app/docker/production-fastapi/

# Deploy on EC2
echo "Deploying application on EC2..."
ssh -i ~/.ssh/deploy_key -o StrictHostKeyChecking=no ubuntu@${FASTAPI_SERVER_HOST} << EOF
set -e
